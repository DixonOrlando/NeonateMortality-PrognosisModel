...
#######################XGBoost with no imputation; prior simplification#####################
...
for (i in unique(final_neo$cluster_code)) {
  ...
  if (i == "1"){
    ...
    metric_measure_clas_xgb_imp <- data.frame(pr_auc = 1,
                                              roc_auc = 1,
                                              log_loss = 1,
                                              brier = 1,
                                              cluster_code = "A")
    
    calibration_measure_xgb_imp <- data.frame(cal_slope = 1,
                                              cal_int = 1,
                                              se_slope = 1,
                                              se_int = 1,
                                              cluster_code = "A")
    
    c_stat_xgb_imp <- data.frame(roc_auc = 1,
                                 se_roc_auc = 1,
                                 cluster_code = "A")
  ...
  set.seed(1111)
  child_res_xg <- vfold_cv(child_train1, 
                           v = 5,
                           strata = Died)
  ...
  xgb_fit <- fit(final_xgb, data = child_train1)
  
  cell_test_pred <- augment(xgb_fit, new_data = child_test1)
  ...
  metric_measure_clas_xgb_imp[row, 1] <- (cell_test_pred %>% pr_auc(Died, .pred_1))[3] #PR-AUC
  metric_measure_clas_xgb_imp[row, 2] <- (cell_test_pred %>% roc_auc(Died, .pred_1))[3] #ROC-AUC
  metric_measure_clas_xgb_imp[row, 3] <- (cell_test_pred %>% mn_log_loss(truth = Died, .pred_1))[3] #log-loss
  metric_measure_clas_xgb_imp[row, 4] <-  (cell_test_pred %>% brier_class(truth = Died, .pred_1))[3] #Brier
  metric_measure_clas_xgb_imp[row, 5] <- i
  
  
  calibration_measure_xgb_imp[row, 1] <- coef(summary(slope_pc))[2,1] #cal_slope
  calibration_measure_xgb_imp[row, 2] <- coef(summary(intercept_pc))[1,1] #cal_int
  calibration_measure_xgb_imp[row, 3] <- coef(summary(slope_pc))[2,2] #se_slope
  calibration_measure_xgb_imp[row, 4] <- coef(summary(intercept_pc))[1,2]#se_int
  calibration_measure_xgb_imp[row, 5] <- i #cluster_code
  
  c_stat_xgb_imp[row, 1] <- pROC::roc(cell_test_pred$Died, cell_test_pred$.pred_1)$auc
  c_stat_xgb_imp[row, 2] <- sqrt(var(pROC::roc(cell_test_pred$Died, cell_test_pred$.pred_1)))
  c_stat_xgb_imp[row, 3] <- i
  
  if (i == "1") {
    p <- unlist(predict(xgb_fit, child_test1, type = "prob")[1])
...
#######################XGBoost with bagged tree imputation; prior simplification#####################
...
######################XGBoost with MICE imputation; prior simplification#####################
...
for (i in unique(final_neo$cluster_code)) {
  ...
  c_stat_xgb_imp[row, 1] <- pROC::roc(cell_test_pred$Died, cell_test_pred$.pred_1)$auc
  c_stat_xgb_imp[row, 2] <- sqrt(pROC::var(pROC::roc(cell_test_pred$Died, cell_test_pred$.pred_1)))
  c_stat_xgb_imp[row, 3] <- i
...
######################XGBoost with bagged tree imputation; after simplification#####################
...
for (i in unique(final_neo$cluster_code)) {
  ...
  xgb_rec <- recipe(Died ~ Q52SP02P + Q48WEIGH + Q50HEART + Q53TEMPE + Q51RESPI + Q37RESPI2,
                    data = child_train1) %>%
    step_impute_bag(all_predictors()) %>%
    step_dummy(all_nominal(), -all_outcomes()) 
